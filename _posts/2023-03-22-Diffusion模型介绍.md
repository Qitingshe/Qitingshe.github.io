...
2. 利用这个噪声 $ \mathbf{ x }_{T} $ 计算出下一个采样 $ \mathbf{ x }_{T-1} $ 的分布，从中采样出 $ \mathbf{x}_{T-1} $;
3. 然后送入模型预测一个噪声 $ \epsilon_{T-1} $;
4. 利用这预测的噪声对 $ \mathbf{x}_{T-1} $ 进行去噪，生成 $ \mathbf{x}_{T-2} $ ，理论上讲 $ \mathbf{x}_{T-2}$ 会更加趋近于真实图像数据的分布。
