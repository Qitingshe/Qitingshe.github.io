...
2. 利用步骤1中的采样噪声计算出下一个采样噪声 $ \mathbf{ x }_{_{T-1}} $ 的分布，从中采样出 $ \mathbf{x}_{T-1} $;
3. 然后送入模型预测一个噪声 $ \epsilon_{T-1} $;
4. 利用模型预测的噪声对 $ \mathbf{x}_{_{T-1}} $ 进行去噪，生成 $ \mathbf{x}_{T-2} $
...
令 $\alpha_t=1-\beta_t$，且 $ \bar{\alpha}{_t} = \prod_{ i=1 }^{t} \alpha_{i} $，可得
